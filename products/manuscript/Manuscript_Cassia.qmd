...
All models based on historical data must contend with limited sample sizes and missing data. In this study, a major limitation was the exclusion of gestational age from the original published clinical notes upon which this dataset is based. A more complete analysis would further explore interactions between birth weight and birth length, which can be a proxy for the nutritional and health status of the mother later in pregnancy [@santana2021]. Lack of data on maternal nutrition and health status, which cannot be corrected for by using race as a proxy for overall health, is a major impediment to understanding the true relationship between maternal variables and infant birth weight in past time periods [@loudon1992, @woods2009]. 
