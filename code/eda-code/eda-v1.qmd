...
We are saving the results to the `results` folder. All final table/figures will have `_final` in the name. This is to make sure we don't overwrite anything we might want to keep.
...
Now let's work with our categorical variables.
...
Now lets get summary statistics of the other categorical variables, including `Status`, `Nationality`, `Birth`, `MaternalOutcome`, `FetalOutcome` (excluding abortion), and `Sex`. We will also create dummy variables for the `Status` and `Nationality` variables for ease of interpretation. For the `Status` variable, we will combine "multigesta" and "trigesta" into "multigesta", since they mean the same thing (more than two pregnancies). For the `Nationality` variable, we will combine all European nationalities into one category, "European", all Latin American nationalities into "LatinAmerican", and all Middle Eastern nationalities into "MiddleEastern". "Brazilian" will remain the same.
...
print(birth_freq)
```
We can see that of a sample size of n = 2761, 88% of outcomes were natural deliveries. Now let's take abortion out of this sample size to just look at labor and delivery outcomes.
...
print(birth_freq2)
```
We can see that of a new sample size of n = 2672, 91% of outcomes were natural deliveries. Now let's look at maternal outcomes.
...
print(maternal_freq)
```
We can see that of the 2829 women who visited the clinic, 23 died, although not necessarily in childbirth. I need to come back to maternal mortality later after I have figured out live births. The equation is MMR = MD/LB * 10,000. We can also look at stillbirth rate. The equation is SBR = SB/LB * 1000.
...
```
The MMR is 94.3% in this specific clinic. The SBR is 84.8% in this specific clinic.

Finally, lets look at infant sex.
...
I will need to come back and calculate sex ratio in the clinic, but I need to first figure out how to exclude stillbirths from the sample, since sex ratio is calculated male/female * 100 for live births only.
