...
# Remove median,maternal age, low brith weight, page numbers, and types of measurement variables given missing data
Mean_Birthweights <- Mean_Birthweights %>% dplyr::select(-Median, -`Maternal Age (mean)`, -"<2500gm (%)", -`Page #`, -"Measurement")
...
```
# Data loading for total fertility rates (TFR)

I will also discuss total fertility rates (TFR) for Rio de Janeiro and Brazil. 

```{r}
# Path to data
# Note the use of the here() package and not absolute paths
data_location <- here::here("data","raw-data","1940_FertilityRates.csv")
Fertility_Rates <- read_csv(data_location)

# Structure of Data
glimpse(Fertility_Rates)

# Save Mean_Birthweight as RDS
save_data_location <- here::here("data","processed-data","Fertility_Rates.rds")
saveRDS(Fertility_Rates, file = save_data_location)
```