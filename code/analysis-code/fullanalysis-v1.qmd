...
whereas BW is birthweight in grams, MA is maternal ancestry (Euro-descent, Afro-descent), and $\epsilon$ is the error term. 
...
```
Our results here are not that different from the model run on all the data. Our adjusted R-squared is 0.007, the same as the full model. The RMSE is 495, which is slightly lower than the full model (502), which shows better performance.
...
```
Performing cross-validation results in an RMSE that is the same as our training model (495), lower than our full model. The r-squared (0.02) is also higher than our training model (0.008) and our full model (0.007), demonstrating better model performance.
...
When running the model on our testing dataset, our results are not significant, given the small sample size. 
...
whereas BW is birthweight in grams, MA is maternal ancestry (Euro-descent, Afro-descent), Gest is gestational status (nulliparous or multiparous), Age is maternal age in years, and $\epsilon$ is the error term. Results are displayed in Table 3.
...
```
Our RMSE is 498 and our r-squared is 0.02.
...
```
Our RMSE is 490 and our r-squared is 0.03, a slightly better performance than the full model.
...
```
Here, our RMSE is 458, an improvement from both the full model and the training model, and our r-squared is 0.03, the same as our training model.
...
```
Running the model on our test data, we see that the only significant result is for the nulliparous status. All other results are insignificant.
...
Our RMSE is 502 and our r-squared is 0.007.
...
Our RMSE is 495 and our r-squared is 0.009, slightly better performance than the full model. Our results are not significantly different from the full model.
...
```
Using cross-validation, our RMSE is 495 and our r-squared is 0.02, a similar performance to our training model.
...
Much like in the other testing models, our results are not significant (except the intercept). Our performance is also worse (RMSE of 518 and r-squared of 0.005).
...
```
Here, our RMSE is 458 and our r-squared is 0.03, a better performance than both the full model and training model.
...
```
Again, most of our results are not significant except the intercept and nulliparous variable given the small sample size.